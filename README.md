<h1 align="center">🚀 保护伞量化交易机器人 V2.8.0</h1>
...
1. **实盘级健壮性升级 (Robustness)**：
   - **T+1 状态强制对账**：系统启动时自动与富途券商柜台进行真实的 `sellable_quantity` 核对，彻底消除 T+1 凌晨定时任务可能带来的单点故障风险。
   - **WebSocket 自愈机制**：实时风控线程 (`ATRStopLossHandler`) 新增周期性保活与断线自动重订阅机制，防止网络闪断导致的监控盲区。
2. **“影子行情”零延迟计算 (Shadow Market Data)**：
   - 策略引擎引入**影子行情更新机制**。在拉取 K 线计算 MACD/RSI/均线等指标前，强制用秒级 Tick 实时现价覆盖/更新 K 线的最后一根柱子，彻底解决传统定时拉取 K 线导致的突破信号滞后问题。
3. **持仓板块集中度控制 (Sector Exposure Control)**：
   - 在资金分配 (`PortfolioManager`) 中新增行业/板块（如 ETF vs 个股）暴露度上限。强制限制单一板块最多占用 60% 的总并发槽位，有效分散同涨同跌的系统性风险。
4. **全自动 AI 进化闭环 (Weekly ML Retraining)**：
   - 将 `LightGBM` 机器学习模型的训练任务无缝接入系统级调度，设定为**每周六中午 12:00** 自动在后台重训练全量市场数据，实现 AI 预测能力的自动迭代更新。
5. **止盈参数收紧与阶梯修复**：
   - 全局追踪止盈容忍回撤从 `3%` 收紧至 **`1.5%`**，微小利润也能稳稳落袋。修复了阶梯止盈中依赖当前利润而非历史最大利润的逻辑 Bug，使护城河判定更为精准。
...
1. **精准盈亏与资产追踪闭环**：
   - 数据库全面升级，`trade_records` 新增单笔卖出交易的 `realized_pnl` (实现盈亏) 和 `pnl_pct` (收益率)。
   - `user_wallets` 新增 `total_assets` (现金+持仓总净值) 和 `total_pnl` (累计盈亏)，实现全自动账本闭环。
2. **交易执行引擎与并发锁重构**：
   - 彻底修复数据库写入的原子性问题，将余额扣减与持仓更新包裹在同一个 `db.commit()` 中，杜绝数据不一致。
   - 优化 `GlobalTradeLock` 作用域，将耗时的 API 数据拉取移出锁外，确保实时止损线程 (ATR Stop-loss) 能够毫秒级响应。
3. **API 频率管理与令牌桶限流**：
   - 引入全新的 Token Bucket (令牌桶) 算法替换粗暴的 `sleep` 延时，完美适配富途 (60次/30秒) 历史K线频率限制，尾盘并发扫描速度提升 300%。
4. **策略深度调优 (防 Whipsaw 打脸)**：
   - **普通股票策略**：新增“保本护城河”，一旦利润曾达 2.5%，强制将止损线上移至成本价+0.5%，确保盈利单不亏损出局。
   - **杠杆ETF策略**：引入金字塔顺势加仓法 (Pyramiding)。首仓仅用 50% 资金，右侧放量突破后动用剩余资金加仓。放宽死叉离场条件，极大增强了“震荡市拿得住，主升浪敢加仓”的能力，显著提升收益率。
   - 完善回测 API 接口 (`api_server.py`)，支持指定 `end_date` 精确时间切片，并在回测自动预加载 60 天指标缓冲期数据。
...
### v2.8.0 (2026-03-30) - 🛡️ 实盘级健壮性与“影子行情”升级
- 🔄 **T+1 强制对账**: 启动时自动与券商柜台核对可卖数量，解决凌晨跑批的单点故障。
- ⚡ **影子行情 (Shadow K-line)**: 策略计算前动态用最新 Tick 覆盖 K 线最后一根，消除指标延迟。
- 🛡️ **板块集中度控制**: 引入单一板块（ETF/个股）最大 60% 槽位占用限制，强制分散风险。
- 🤖 **AI 自动进化**: 将 `LightGBM` 模型训练任务接入调度，每周六自动迭代。
- 🔧 **WebSocket 自愈**: 增强富途行情订阅机制，断线重连后自动恢复监听。

### v2.7.0 (2026-03-26) - 🛡️ 盈亏追踪与抗打脸策略升级
- 💰 **精准账本**: 数据库新增 `realized_pnl` 与 `total_assets` 追踪，实现卖出盈亏与全局资产自动核算闭环。
- ⚡ **并发与锁优化**: 重构全局交易锁，释放网络请求阻塞，提升实时止损响应速度。修复数据库原子性提交 Bug。
- 🌊 **防打脸策略 (Whipsaw)**: 股票策略加入“2.5%利润保本护城河”。杠杆ETF加入“金字塔加仓法”，并放宽震荡期均线死叉容忍度，大幅提升胜率。
- ⏳ **智能限流**: 引入 Token Bucket 令牌桶算法，完美适配富途 60/30s 频率限制。
